...
Store integration credentials in server-side secrets:

```text
GEOAPIFY_API_KEY=...
DISTANCE_PROVIDER=osrm
# Optional. Leave blank to use the public OSRM demo server while testing.
OSRM_BASE_URL=https://router.project-osrm.org
```

Geoapify supplies UK address autocomplete and forward geocoding. OSRM can
provide free road-based travel times for route planning. The public OSRM demo
server is useful for testing, but a production app should use a hosted/private
OSRM server. A future Google driving-time key must use the server-only
`GOOGLE_ROUTES_API_KEY` name and must never be exposed through an
